...
## goal by tues 
1. run engagement model with cannibalization feature 
2. think about pre-launch retention impact 
3. actives 
4. Cannibalization 
...
sum(eligibles), 
sum(paid_cancels),
sum(paid_cancels) / sum(eligibles)
from MAX_DEV.WORKSPACE.FORECASTING_PROJ_CANCELS_FUTURE 
where region = 'NORTH AMERICA'
and expire_date::date between '2024-10-01' and '2024-12-20'
and tenure<3
group by 1,2 
order by 1,2
"""
...
df_raw.head()
# df_m[df_m.duplicated(subset=['imdb_id','season_number','title_name','region','offering_start_date','days_on_max'], keep=False)]



# COMMAND ----------

df_raw.groupby(by=['region','forecast_category']).nunique()
...
model_list = {
    ...
    # 'pay_dmgp': 'ln_bo_opening_imputed + ln_production_cost + medal + ln_gap + count_platinum_binary',
    # 'pay_dmgpd': 'ln_bo_opening_imputed + ln_production_cost + medal + ln_gap + ln_days_diff + count_platinum_binary',
...
display(df_coef[(df_coef.region=='NORTH AMERICA') & (df_coef.model=='pay_amgp')])
display(df_coef[(df_coef.region=='NORTH AMERICA') & (df_coef.model=='pay_amgpd')])
...
df = pd.read_sql("""
    ...
    sum(paid_cancels) as paid_cancel, 
    sum(paid_cancels) / sum(eligibles) 
    from MAX_DEV.WORKSPACE.FORECASTING_PROJ_CANCELS_FUTURE 
    where region = 'NORTH AMERICA'
...
df.columns=df.columns.str.lower()
display(df)
print(df[0:4].eligibles.sum()/1000000)
print(df[1:5].eligibles.sum()/1000000)
print(df[2:6].eligibles.sum()/1000000)
print(df[3:7].eligibles.sum()/1000000)
# df.date=pd.to_datetime(df.date)
# df[(df.date=='2024-01-02') & (df.payment_period=='PERIOD_MONTH')]
...
df = pd.read_sql("""
    ...
    week_end,
    sum(eligibles) as eligibles, 
    sum(paid_cancels) das paid_cancel, 
    sum(paid_cancels) / sum(eligibles) 
    from MAX_DEV.WORKSPACE.FORECASTING_PROJ_CANCELS_FUTURE 
    where region = 'NORTH AMERICA'
    and week_end::date between '2024-12-05' and '2025-01-07'
    and tenure=1
    group by all 
    order by 1
    """, 

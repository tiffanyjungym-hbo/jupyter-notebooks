...
def get_days_diff(df_in):
    df_in['first_release_date_ref'] = df_in['first_release_date'] - pd.DateOffset(days=60) 
    ...
    for index, row in df_in.iterrows(): 
        for filter in [['Gold','Platinum'], ['Platinum']]: 
...
for i in ['NORTH AMERICA','LATAM','EMEA']:
    ...
    df_s.append(df_r)
    display(df_r[['region','title_name','medal','first_release_date','last_release_date','first_release_date_ref','days_diff', 'prev_title']].sort_values(by=['first_release_date']))

df_s = pd.concat(df_s)

# a = df_s[df_s.region=='NORTH AMERICA'].reset_index()
# display(a[70:81][['region','title_name','medal','first_release_date','last_release_date','last_release_date_ref','days_diff', 'prev_title']].sort_values(by=['first_release_date']))
# display(a[90:95][['region','title_name','medal','first_release_date','last_release_date','last_release_date_ref','days_diff', 'prev_title']].sort_values(by=['first_release_date']))
...
display(df_coef[(df_coef.region=='NORTH AMERICA') & (df_coef.model=='pay_amgdp')])
display(df_coef[(df_coef.region=='LATAM') & (df_coef.model=='pay_cmgd')])
display(df_coef[(df_coef.region=='EMEA') & (df_coef.model=='pay_dmgd')])
...
df_ss2.sort_values(by=['first_release_date'])

# COMMAND ----------

df_ss[(df_ss.medal=='Platinum') | (df_ss.forecast_category=='wb pay1')]
...
df_test.last_release_date =  pd.to_datetime(df_test.last_release_date).dt.date      
display(df_test[['title_name','first_release_date','last_release_date','gap','days_diff','fv_pre_7_pay1','prev_title',]])
display(df_test[['title_name','first_release_date_2','last_release_date_2','gap_2','days_diff_2','fv_pre_7_pay1_2','prev_title',]])
coef

# COMMAND ----------

df_sc_fin[df_sc_fin.title_name=='Civil War']

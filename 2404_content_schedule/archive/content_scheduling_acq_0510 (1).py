...
def get_days_diff(df_in):
    ...
    for index, row in df_in.iterrows(): 
        for filter in [['Gold','Platinum'], ['Platinum']]: 
            ...
            ## days_diff = Difference between the last date of previous title and the release date of current title. If previous title ends after current, days_diff would be negative. This value is winsorized at -28.  
...
## Remove outliers; Batman: 150K fv; Magic Mike: 3K fv; House Party: ~0 
...
df_in = df_in[((df_in.region=='NORTH AMERICA') & ~(df_in.title_name.isin(title_removed))) |
              ((df_in.region=='EMEA') & ~(df_in.title_name.isin(title_removed))) | 
              ((df_in.region=='LATAM') & ~(df_in.title_name.isin(title_removed)))]
...
display(df_coef[(df_coef.region=='EMEA') & (df_coef.model=='pay_ogmdpc')]) 

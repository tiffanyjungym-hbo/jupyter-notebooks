...
def get_days_diff(df_in):
    ...
    for index, row in df_in.iterrows():
        ## Eligibility: 1) released before current title, 2) ended before 28d post premiere of current title
        ...
        ## Eligibility_platinum: 1) released before current title, 2) release within the past 28d 
        eligible_rows_plat = df_in[((df_in['medal'].isin(['Platinum'])) 
                    & (df_in['first_release_date'] < row['first_release_date']))]
        
        if row.title_name=='Elvis':
            display(eligible_rows_plat)
        if not eligible_rows_plat.empty:
            # latest_first_releast_date = eligible_rows_plat['first_release_date'].max()
            # latest_date = eligible_rows_plat.loc[eligible_rows_plat['first_release_date']==latest_first_releast_date, 'last_release_date'].iloc[0]

...
def get_coef(model, model_name, target, region):
    # Model coefficients
        ...
        return df_coef[['region','model','target','feature','coef','tstat','pvalue']]
...
## Remove outliers; Batman: 150K fv; Magic Mike: 3K fv; House Party: ~0  ## color purple emea & latam, blue beetle 
# title_removed = ['The Batman',"Magic Mike's Last Dance"]#'The Batman','House Party', "Magic Mike's Last Dance"
# title_removed_LATAM = ['House Party','The Batman',  "Magic Mike's Last Dance",'DC League of Super-Pets']#,'The Batman','Barbie']#, 'Blue Beetle']
# title_removed_EMEA = ['House Party','The Batman',  'DC League of Super-Pets','Evil Dead Rise']
...
target = 'rfv7'
model_list = {
    # 'pp':'medal + ln_days_diff + ln_gap',
    # 'pp':'medal + ln_days_diff + ln_gap + ln_ref_count',
    # 'ppr':'medal + ln_days_diff + ln_gap + ln_ref_popularity',
    # 'pay_o':'ln_bo_opening_imputed',
    # 'pay_og':'ln_bo_opening_imputed + ln_gap',
    # 'pay_ogf':'ln_bo_opening_imputed + ln_gap + ln_fv_pre_7',
    # 'pay_ogd':'ln_bo_opening_imputed + ln_gap + ln_days_diff',
    # 'pay_ogdc': 'ln_bo_opening_imputed + ln_gap + ln_days_diff + ln_production_cost',
    # 'pay_ogdm': 'ln_bo_opening_imputed + ln_gap + ln_days_diff + medal',
    # 'pay_ogdp': 'ln_bo_opening_imputed + ln_gap + ln_days_diff + ln_fv_pre_7',
    # 'pay_ogdf': 'ln_bo_opening_imputed + ln_gap + ln_days_diff + ln_prev_title_fv',
    # 'pay_ogfs': 'ln_bo_opening_imputed + ln_gap + ln_fv_pre_7_series',
    # 'pay_ogfp': 'ln_bo_opening_imputed + ln_gap + ln_fv_pre_7_pay1',
    'pay_ogdmc': 'ln_bo_opening_imputed + ln_gap + ln_days_diff + medal + ln_production_cost',
    'pay_ogmdfc': 'ln_bo_opening_imputed + ln_gap + medal + ln_days_diff  + ln_fv_pre_7_pay1 + ln_production_cost',
    'pay_ogmdfsc': 'ln_bo_opening_imputed + ln_gap + medal + ln_days_diff   + ln_fv_pre_7_pay1 + ln_fv_pre_7_series + ln_production_cost',
    'pay_ogdmfct': 'ln_bo_opening_imputed + ln_gap + medal + ln_days_diff  + ln_fv_pre_7 + ln_production_cost',
    # 'pay_ogdfc': 'ln_bo_opening_imputed + ln_gap + ln_days_diff  + ln_fv_pre_7_pay1 + ln_production_cost',
    # 'pay_ogfsmc': 'ln_bo_opening_imputed + ln_gap + ln_fv_pre_7_series + medal + ln_production_cost',
    # 'pay_ogfpmc': 'ln_bo_opening_imputed + ln_gap + ln_fv_pre_7_pay1 + medal + ln_production_cost',
    # 'pay_ogfmc': 'ln_bo_opening_imputed + ln_gap + ln_fv_pre_7 + medal + ln_production_cost'
}
    # 'pay_d':'ln_days_diff',
    # 'pay_dm':'ln_days_diff + medal',
    # 'pay_dmo':'ln_days_diff + medal + ln_bo_opening_imputed',
    # 'pay_dmog':'ln_days_diff + medal + ln_bo_opening_imputed + ln_gap',
    # 'pay_dmogc':'ln_days_diff + medal + ln_bo_opening_imputed + ln_gap + ln_production_cost',
    # 'pay_dmogcp':'ln_days_diff + medal + ln_bo_opening_imputed + ln_gap + ln_production_cost + fv_pre_7'
...
df_fv_ = get_rolling_sum(df_fv, ['fv_retail','fv_series','fv_pay1'])
df_fv_['date'] = df_fv_['date'] - pd.to_timedelta(1, unit='D') ## shift date to exclude today from rolling

## Get scenario 2 (gxk release 2 07/05-> 06/14, 21d shift)
...
df_fv_title_new['date'] = df_fv_title_new['date'] + pd.to_timedelta(-21, unit='D')
df_fv_title_new['date'] = pd.to_datetime(df_fv_title_new['date'])

## Get rolling first views by date, for scenario 1 vs 2 
df_fv_sc = df_fv_.merge(df_fv_title, on=['date'],how='left').merge(df_fv_title_new, on=['date'],how='left', suffixes=('','_2'))
df_fv_sc= df_fv_sc.fillna(0)
df_fv_sc['fv_pay1_2'] = df_fv_sc['fv_pay1'] - df_fv_sc['fv_title'] + df_fv_sc['fv_title_2']
df_fv_sc['fv_series_2'] = df_fv_sc['fv_series']
df_fv_sc['fv_retail_2'] = df_fv_sc['fv_retail']  - df_fv_sc['fv_title'] + df_fv_sc['fv_title_2']
...
df_sc_fin = df_sc.merge(df_fv_sc[['date','fv_pay1','fv_series', 'fv_retail']], left_on ='premiere_date', right_on='date', how='left')
df_sc_fin = df_sc_fin.merge(df_fv_sc[['date','fv_pay1_2', 'fv_series_2','fv_retail_2','delta_fv_pre_7_pay1']], left_on ='first_release_date_2', right_on='date', how='left')
df_sc_fin = df_sc_fin.sort_values(by='premiere_date')

df_sc_fin = df_sc_fin[['title_name', 'season_number', 'imdb_id', 'premiere_date', 'forecast_category', 'rfv7', 'medal', 'num_episodes','first_release_date', 'last_release_date', 'bo_opening_imputed','production_cost','prev_title', 'prev_title_fv', 'gap', 'days_diff', 'fv_pay1', 'fv_series', 'fv_retail','first_release_date_2', 'last_release_date_2', 'prev_title_2', 'gap_2',  'days_diff_2', 'fv_pay1_2', 'fv_series_2','fv_retail_2', 'delta_days_diff', 'delta_gap',   'delta_fv_pre_7_pay1']]
...
coeff_val={}
for i,j in zip(['ln_days_diff'],['c_days_diff']):
    coeff_val[j] = coef[coef.feature==i].at[coef[coef.feature==i].index[0],'coef']
...
coef = df_coef[df_coef.model=='pay_ogmdfsc'].copy()
display(coef)
list_val = ['days_diff','fv_pre_7_pay1','fv_pre_7_series','gap']

coeff_val={}
for i in list_val:
    coeff_val[i] = coef[coef.feature=='ln_'+i].at[coef[coef.feature=='ln_'+i].index[0],'coef']

# coeff_pre_7_pay1 = coef[coef.feature=='ln_days_diff'].at[coef[coef.feature=='ln_days_diff'].index[0],'coef']
# coeff_pre_7_pay1 = coef[coef.feature=='ln_fv_pre_7_pay1'].at[coef[coef.feature=='ln_fv_pre_7_pay1'].index[0],'coef']
# coeff_pre_7_series = coef[coef.feature=='ln_fv_pre_7_series'].at[coef[coef.feature=='ln_fv_pre_7_series'].index[0],'coef']
# coeff_gap = coef[coef.feature=='ln_gap'].at[coef[coef.feature=='ln_gap'].index[0],'coef']


df_test = df_sc_fin[(df_sc_fin.forecast_category=='wb pay1')]

for i in list_val:
    df_test['fv_change_{}'.format(i)]= np.exp(df_test[i] * i)/(df_test.days_diff_2**coeff_days_diff)-1

    EXP(LN(I34)*M27)/EXP(LN(I28)*M27)
df_test['fv_change_days_diff']= (df_test.days_diff**coeff_days_diff)/(df_test.days_diff_2**coeff_days_diff)-1
df_test['fv_change_pre_7_pay1']= (df_test.fv_pay1**coeff_pre_7_pay1)/(df_test.fv_pay1_2**coeff_pre_7_pay1)-1
df_test['fv_change_ln_fv_pre_7_series']= (df_test.fv_series**coeff_pre_7_series)/(df_test.fv_series_2**coeff_pre_7_series)-1
df_test['fv_change_gap']= (df_test.gap_2**coeff_gap)/(df_test.gap**coeff_gap)-1
df_test['fv_change'] =  
...
corr_matrix = df_in[df_s.forecast_category=='series'][['ln_rfv7','ln_days_diff','ln_search_2wbp','ln_wiki_2wbp','ln_num_episodes','ln_fv_pre_7','ln_ref_count','ln_ref_popularity']].corr()
...
df_vif = df_s[df_s.forecast_category=='series'][['search_2wbp', 'fv_pre_7','days_diff','ref_count', 'ref_popularity']]
vif_data = pd.DataFrame()
vif_data["feature"] = df_vif.columns
vif_data["VIF"] = [variance_inflation_factor(df_vif.values, i) for i in range(df_vif.shape[1])]
